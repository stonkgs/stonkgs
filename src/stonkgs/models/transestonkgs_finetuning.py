...
from stonkgs.constants import (
    ...
    NLP_MODEL_TYPE,
    ORGAN_DIR,
    PRETRAINED_TRANSESTONKGS_PATH,
    RELATION_TYPE_DIR,
    SPECIES_DIR,
    TRANSE_EMBEDDINGS_PATH,
    TRANSE_STONKGS_OUTPUT_DIR,
    VOCAB_FILE,
)
from stonkgs.models.kg_baseline_model import prepare_df
from stonkgs.models.stonkgs_finetuning import INDRADataset
...
def preprocess_fine_tuning_data(
    ...
    unprocessed_df = pd.read_csv(
        train_data_path,
        sep="\t",
        usecols=["source", "relation", "target", "evidence", class_column_name],
    )
    ...
    for _, row in tqdm(
        ...
        # 1. "Token type IDs": 0 for text tokens, 1 for entity/relation tokens
        ...
        # 3. Get the node/relation ID sequence, add the SEP (usually with id=102) afterwards
        entity_ids = (
        ...
        # 5. Total input_ids = 256 text ids + 4 entity/relation ids
        input_ids = text_token_ids + entity_ids
...
class TransESTonKGsForSequenceClassification(TransESTonKGsForPreTraining):
    """Create the fine-tuning part of the TransESTonKGs model based the pre-trained TransESTonKGs model.

    Note that this class inherits from TransESTonKGsForPreTraining rather than PreTrainedModel, thereby it's deviating
    from the typical huggingface inheritance logic of the fine-tuning classes.
    """
    ...
    def forward(
        ...
        ent_embeddings = torch.stack(
            [
                ...
                for j in input_ids[:, self.cls.predictions.text_part_length :]
            ],
        )
...
def run_sequence_classification_cv(
    train_data_path: str,
    model_path: str = PRETRAINED_TRANSESTONKGS_PATH,
    output_dir: Optional[str] = TRANSE_STONKGS_OUTPUT_DIR,
    ...
    mlflow.set_experiment("TransESTonKGs Fine-Tuning")

    # Initialize a dataframe for all the predicted labels
    result_df = pd.DataFrame()

    for idx, indices in enumerate(train_test_splits):
        model = TransESTonKGsForSequenceClassification.from_pretrained(
    ...
    result_df.to_csv(
        os.path.join(
            TRANSE_STONKGS_OUTPUT_DIR, "predicted_labels_transestonkgs_" + task_name + "df.tsv"
        ),
        index=False,
        sep="\t",
    )
    ...
    # Save the last model
    trainer.save_model(output_dir=os.path.join(TRANSE_STONKGS_OUTPUT_DIR, task_name))
...
@click.option(
    "--model_path",
    default=PRETRAINED_TRANSESTONKGS_PATH,
    help="Path of the pretrained model",
    type=str,
)
@click.option("--output_dir", default=TRANSE_STONKGS_OUTPUT_DIR, help="Output directory", type=str)
...
def run_all_fine_tuning_tasks(
    ...
    lr: float = 5e-5,
    model_path: str = PRETRAINED_TRANSESTONKGS_PATH,
    output_dir: str = TRANSE_STONKGS_OUTPUT_DIR,
