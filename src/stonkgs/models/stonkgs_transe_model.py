...
from stonkgs.constants import NLP_MODEL_TYPE, TRANSE_EMBEDDINGS_PATH
from stonkgs.models.kg_baseline_model import prepare_df
from stonkgs.models.stonkgs_model import BertForPreTrainingOutputWithPooling
...
class TransESTonKGsELMPredictionHead(BertLMPredictionHead):
    ...
    def forward(self, hidden_states):
        """Map hidden states to values for the text vocab (first part) and kg vocab (second part)."""
        ...
        text_hidden_states_to_vocab = self.text_decoder(hidden_states[:, : self.text_part_length])
        ent_hidden_states_to_kg_vocab = self.entity_decoder(
            hidden_states[:, self.text_part_length :]
        )

        return text_hidden_states_to_vocab, ent_hidden_states_to_kg_vocab


class TransESTonKGsForPreTraining(BertForPreTraining):
    ...
    def __init__(
        ...
        kg_embedding_dict_path: str = TRANSE_EMBEDDINGS_PATH,
    ):
        ...
        self.cls.predictions = TransESTonKGsELMPredictionHead(config)
